package com.hyunnyapp.brainyproject.brainycar.controler.cam;
...
{	
	...
	public Bitmap getPreview(int x, int y, int width, int height, int quality) 
	{
		//System.gc(); 
		Bitmap b = null;
		...
		yuvimage.compressToJpeg(new Rect(x, y, width, height), quality, outStream); // make JPG
		b = BitmapFactory.decodeByteArray(outStream.toByteArray(), 0, outStream.size()); // decode JPG
		if (b != null) 
		{
			if(Constants.UIDEBUG) Log.i(TAG, "getPic() WxH:" + b.getWidth() + "x" + b.getHeight());
		} 
		else 
		{
			if(Constants.UIDEBUG) Log.i(TAG, "getPic(): Bitmap is null..");
		}
		yuvimage = null;
		outStream = null;
		//System.gc();
	...
	private void updateBufferSize() 
	{
		...
		if(Constants.UIDEBUG) Log.i("surfaceCreated", "buffer length is " + mBuffer.length + " bytes");
	}

	public void surfaceCreated(SurfaceHolder holder) {
		// The Surface has been created, acquire the camera and tell it where to draw.
		if(Constants.UIDEBUG) Log.i(TAG, "surfaceCreated()");
		try 
		{
			mCamera = Camera.open(); // WARNING: without permission in Manifest.xml, crashes
		}
		catch (RuntimeException exception) {
			if(Constants.UIDEBUG) Log.i(TAG, "Exception on Camera.open(): " + exception.toString());
			...
			mCamera.setPreviewCallbackWithBuffer(new PreviewCallback() 
			{
				public synchronized void onPreviewFrame(byte[] data, Camera c) 
				{
					...
					{ // there was a race condition when onStop() was called..
						//Log.i(TAG, "PreviewCallBack onPreviewFrame()");
		...
		{
			if(Constants.UIDEBUG) Log.e(TAG, "Exception trying to set preview");
			if (mCamera != null)
			{
				mCamera.release();
				mCamera = null;
	...
	public void surfaceDestroyed(SurfaceHolder holder) 
	{
		...
		if(Constants.UIDEBUG) Log.i(TAG,"SurfaceDestroyed being called");
		mCamera.stopPreview();
		mCamera.release();
		mCamera = null;
	}
	...
	{
		if(Constants.UIDEBUG) Log.i(TAG, "Preview: surfaceChanged() - size now " + w + "x" + h);
		...
		try
		{
			...
			{
				if(Constants.UIDEBUG) Log.i(TAG, "supported preview format: " + i);
			} 

			List<Size> sizes = mParameters.getSupportedPreviewSizes();
			for (Size size : sizes) 
			{
				if(Constants.UIDEBUG) Log.i(TAG, "supported preview size: " + size.width + "x" + size.height);
			}
		...
		if(Constants.UIDEBUG) Log.i(TAG, "Preview: checking it was set: " + p.width + "x" + p.height); // DEBUG
