...
//            setParameters(DEFAULT_BAUD_RATE, DEFAULT_DATA_BITS, DEFAULT_STOP_BITS, DEFAULT_PARITY);
    ...
    public boolean writeOneByte(byte src, int timeoutMillis) throws IOException {
    	byte[] writeSrc = new byte[1];
    	writeSrc[0] = src;
    	
        final int amtWritten = mConnection.bulkTransfer(mWriteEndpoint, writeSrc, writeSrc.length, timeoutMillis);

        if (amtWritten <= 0) {
            throw new IOException("Error writing length=" + writeSrc.length);
        }

        Log.d(TAG, "Wrote amt=" + amtWritten);

        return true;
    }

    public boolean writeOneByte(byte[] src, int timeoutMillis) throws IOException {
        final int amtWritten = mConnection.bulkTransfer(mWriteEndpoint, src, src.length, timeoutMillis);

        if (amtWritten <= 0) {
            throw new IOException("Error writing length=" + src.length);
        }
