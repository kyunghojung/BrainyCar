...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
    ...
    public boolean writeOneByte(byte src, int timeoutMillis) throws IOException {
        ...
        final int amtWritten = mConnection.bulkTransfer(endpoint, writeSrc, writeSrc.length, timeoutMillis);
    ...
    public boolean writeOneByte(byte[] src, int timeoutMillis) throws IOException {
        final UsbEndpoint endpoint = mDevice.getInterface(0).getEndpoint(1);
        final int amtWritten = mConnection.bulkTransfer(endpoint, src, src.length, timeoutMillis);
